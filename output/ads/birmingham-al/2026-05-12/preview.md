# Ad creative preview — Birmingham

Generated 2026-05-12. Example output produced from the top-impact approved story.

## Source story

- Slug: `the-committee-will-consider-an-agreement-with-jefferson-county-to-release-a-rest-fc0f18`
- Category: Budget · Impact: High
- Story ID: `fc0f1868-ffec-4193-9882-e837a085b8af`

The committee will consider an agreement with Jefferson County to release a restrictive covenant and reversionary interest on the former Graymont School property. In exchange, the city would agree to pay the county $790,000 — representing the fair market value of the county's 50% interest — if the city no longer owns the property or ceases to use it for a public purpose on or before April 21, 2032.

## Creative variants

### civic_fact
**Birmingham weighs $790K covenant deal**
Council to consider releasing covenant on Graymont School property.
CTA: Read story

### resident_impact
**City may owe $790K by 2032**
Conditions on a former school site could trigger a county payout.
CTA: Learn more

### curiosity
**Why $790K? What's the catch?**
A covenant on the former Graymont School comes with a price tag.
CTA: See details

## Targeting

```json
{
  "geo": { "city": "Birmingham", "state": "Alabama", "radius_miles": 5 },
  "age": { "min": 28, "max": 65 },
  "interests": ["local news", "city government", "Birmingham Alabama", "civic engagement", "real estate", "homeowner"],
  "behaviors": ["small business owner", "registered voter"],
  "exclude": ["users who clicked this campaign in last 30 days"]
}
```

## Per-platform destinations

- **facebook**: https://birmingham-al-civic.vercel.app/posts/the-committee-will-consider-an-agreement-with-jefferson-county-to-release-a-rest-fc0f18?utm_source=facebook&utm_medium=cpc&utm_campaign=birmingham-al_the-committee-will-consider-an_2026-05-12
- **nextdoor**: https://birmingham-al-civic.vercel.app/posts/the-committee-will-consider-an-agreement-with-jefferson-county-to-release-a-rest-fc0f18?utm_source=nextdoor&utm_medium=cpc&utm_campaign=birmingham-al_the-committee-will-consider-an_2026-05-12

## Suggested budget

$20/day × 5 days = $100 total on Facebook. $15/day × 5 days = $75 total on Nextdoor (lower CPC for civic content). Adjust in `payload.json` before submission.

## Reviewer checklist

Before submitting these to Facebook or Nextdoor, confirm:

- [ ] All three variant headlines and bodies are factually accurate to the source story.
- [ ] No partisan framing, no emotional escalation, no clickbait.
- [ ] Destination URLs resolve to a real story page on `birmingham-al-civic.vercel.app`.
- [ ] Budget matches what you intended to spend.
- [ ] Geo, age, and interests look reasonable for Birmingham.

Then follow `docs/AD_SUBMISSION.md` to push to each platform.
